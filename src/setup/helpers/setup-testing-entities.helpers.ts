...
import { Services, SetupConfig, SetupData } from '../interfaces/setup-services.interface';
...
export class SetupTestingEntitiesHelpers {
  ...
  async createTestingData(config: SetupConfig): Promise<SetupData> {
    ...
    if (config.organizations && config.organizations.count > 0) {
      organizations = await this.createTestingOrganization(config.organizations.count);
    }

    let orders: GetOrderDto[] = [];
    if (config.orders && config.orders.count > 0) {
      orders = await this.createTestingOrders(superAdmin, organizations[0].id, config.orders.count);
    }

    let admins: GetUserDto[];
    admins = await this.createTestingAdmins(superAdmin, organizations[0].id, config.users.count, config.testName);
  ...
  async createTestingOrganization(count: number): Promise<GetOrganizationDto[]> {
  ...
  async createTestingAdmins(
    ...
    for (let i = 0; i < count; i++) {
      ...
      const admin = await this.usersService.createAdmin(createUserDto, organizationId);
